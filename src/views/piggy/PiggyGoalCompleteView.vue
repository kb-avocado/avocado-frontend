    ...
    <div class="min-h-screen flex flex-col bg-white">
        ...
        <div class="flex-1 p-4 space-y-6">
            ...
            <div class="rounded-2xl border border-avocado-100 shadow-sm p-4 text-center space-y-2">
                ...
                <p class="text-lg font-bold text-avocado-900">{{ goal.title }}</p>
...
async function handleSubmit() {
    ...
    try {
        // 응원 메세지 보내기 함수 활용
        await sendCheerMessage(goal.value.id, { message: message.value })

        // 메세지 전송 성공 후 송금하기 페이지로 갈 때 가지고 갈 파라미터
        router.push({
            name: 'piggyBonusTransferStub',
            params: { id: goal.value.id },
            query: { amount: goal.value.bonusAmount }
        })
    } catch (e) {
