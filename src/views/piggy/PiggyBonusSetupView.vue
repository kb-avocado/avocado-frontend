        ...
        <div class="flex-1 p-4 space-y-6">
            ...
            <div>
                ...
                <div class="flex rounded-full bg-avocado-50 border border-avocado-300 p-1">
                    <button type="button" class="flex-1 h-9 rounded-full text-sm font-medium transition-colors"
                        :class="bonusType === BONUS_TYPE.RATE ? 'bg-avocado-100 text-avocado-600' : 'text-muted'"
                        @click="selectBonusType(BONUS_TYPE.RATE)">
                        응원 보너스 설정
                    </button>
                    <button type="button" class="flex-1 h-9 rounded-full text-sm font-medium transition-colors"
                        :class="bonusType === BONUS_TYPE.FIXED ? 'bg-avocado-100 text-avocado-600' : 'text-muted'"
            ...
            <div v-if="bonusType">
                ...
                <div class="flex items-center border border-avocado-300 rounded-xl px-3 h-11">
                    <input v-model="bonusValue" type="number" inputmode="numeric"
                ...
                <p v-if="bonusValue && !isValueValid" class="text-xs text-red-500 mt-1">
                    {{ bonusType === BONUS_TYPE.RATE ? '1~100 사이의 숫자를 입력해주세요.' : '0보다 큰 금액을 입력해주세요.' }}
                </p>
            ...
            <div class="flex items-start gap-2 bg-avocado-50 rounded-xl p-4">
                ...
                <p class="text-xs text-avocado-900 leading-relaxed">
                    보호자님은 보너스 또는 지원금 중 하나를 선택하여 아이를 응원할 수 있습니다.
                    한 번 설정하면 이후에는 변경할 수 없으니, 설정을 완료하면 승인 버튼을 눌러주세요.
                </p>
            </div>
        </div>
