        ...
        <div class="flex-1 p-4 space-y-6" v-if="item">
            <div class="rounded-2xl bg-avocado-100 p-4 text-center space-y-1">
                ...
                <p class="text-xs text-muted">
                    남은 금액 <span class="font-semibold text-avocado-900">{{ formatCurrency(remainingAmount) }}</span>
                </p>
            </div>
...
async function handleSubmit() {
    ...
    try {
        ...
        router.push({ name: 'piggyChildDetail', params: { id: piggyBankId.value } })
    } catch (e) {
        errorMessage.value =
            e.response?.data?.message || '저금에 실패했어요. 다시 시도해주세요.'
    } finally {
