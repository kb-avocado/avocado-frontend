<template>
  ...
  <div v-else class="p-4 flex flex-col gap-6">
    ...
    <Transition name="fade" mode="out-in">
      ...
      <div v-else-if="!isLoading" class="p-4 text-center py-10">
...
// 응답이 너무 빨리 오면 로딩 화면이 깜빡이므로 최소한 이만큼은 로딩 상태를 유지한다.
...
async function fetchHome() {
  ...
  try {
    ...
    const remaining = MIN_LOADING_MS - (Date.now() - loadingStartedAt)
    if (remaining > 0) {
...
onMounted(async () => {
  // authStore.refresh()가 끝나기 전까지 home이 null이라 오류 화면으로 오인될 수 있으므로
  // 미리 로딩 상태로 표시해 둔다.
  isLoading.value = true
