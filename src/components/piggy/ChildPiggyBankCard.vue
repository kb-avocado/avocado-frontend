<template>
  <article
    ...
    <header class="min-w-0 grid grid-cols-[46px_minmax(0,1fr)_auto] items-center gap-[11px]"
      ...
      <!-- 아이 화면: 즐겨찾기(하트) 토글 -->
      <button type="button" class="p-[7px] border-0 bg-transparent cursor-pointer"
        :aria-pressed="Boolean(item.favorite)" :aria-label="item.favorite ? '즐겨찾기 등록됨' : '즐겨찾기 등록 안 됨'"
    ...
    <div class="grid gap-[9px]" :class="{ 'opacity-[0.42]': isFinished }">
      ...
      <div class="w-full h-2.5 overflow-hidden rounded-full" style="background-color: #ebebeb">
        <div class="h-full rounded-full transition-[width] duration-700 ease-out"
          :style="{ width: revealed ? `${safeRate}%` : '0%', backgroundColor: progressColor }"></div>
      </div>
    </div>

    <section v-if="isPendingAchieve" class="min-h-[62px] py-3 px-[14px] flex items-start gap-2 rounded-[13px] bg-white">
      ...
      <p v-if="hasBonus" class="text-[12px] leading-relaxed" style="color: #555353">
        <strong style="color: #e1585a">{{ dday }}일만 더</strong> 기다리면 모은 돈을
        돌려받을 수 있어요.<br />
        보너스 <strong style="color: #4e9440">{{ bonusText }}</strong>도 곧 받을 수 있어요!
      </p>
